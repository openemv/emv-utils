...
static bool emv_date_is_valid(const struct emv_tlv_t* txn_date)
{
	return
		txn_date &&
		txn_date->length == 3 &&
		txn_date->value &&
		(txn_date->value[0] >> 4) <= 9 &&
		(txn_date->value[0] & 0xF) <= 9 &&
		txn_date->value[1] != 0 &&
		txn_date->value[2] != 0 &&
		emv_date_format_n_to_uint8(txn_date->value[1]) <= 12 &&
		emv_date_format_n_to_uint8(txn_date->value[2]) <= 31;
}
